    ...
    <div className="space-y-8 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        ...
        <button 
          ...
          className="bg-[#ff3600] text-white px-8 py-4 rounded-full text-sm font-bold shadow-xl shadow-[#ff3600]/20 hover:scale-105 transition-transform flex items-center gap-3"
        >
          <Plus size={20} />
          Add Booking
        </button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-6">
          ...
          <div>
            ...
            <select
              ...
              className="w-full px-4 py-3 rounded-2xl border border-gray-200 focus:border-[#ff3600] focus:ring-2 focus:ring-[#ff3600]/20 outline-none transition-all"
            >
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 rounded-2xl border border-gray-200 focus:border-[#ff3600] focus:ring-2 focus:ring-[#ff3600]/20 outline-none transition-all"
              />
            </div>
          </div>
          ...
          <div className="flex items-end">
            <button className="w-full px-6 py-3 rounded-2xl border-2 border-gray-200 hover:border-[#ff3600] text-gray-600 hover:text-[#ff3600] font-bold transition-all flex items-center justify-center gap-2">
              <Download size={18} />
              Export
            </button>
          </div>
                ...
                <tr key={assignment.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-2">
                      <button onClick={() => openEditModal(assignment)} className="p-2 hover:bg-gray-100 rounded-xl text-gray-600 hover:text-[#ff3600] transition-colors">
                        <Edit2 size={16} />
                      </button>
            ...
            <div className="relative bg-white rounded-[2rem] p-8 w-full max-w-lg shadow-2xl animate-in zoom-in-95 pointer-events-auto max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <input type="text" value={assignmentForm.routeName || ''} onChange={e => setAssignmentForm({...assignmentForm, routeName: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]" placeholder="e.g. Route A - North" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-600 mb-1">School</label>
                  <input type="text" value={assignmentForm.school || ''} onChange={e => setAssignmentForm({...assignmentForm, school: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]" placeholder="e.g. Greenfield Academy" />
                </div>
                <div>
                  <label className="block text-sm font-bold text-gray-600 mb-1">Driver</label>
                  <input type="text" value={assignmentForm.driver || ''} onChange={e => setAssignmentForm({...assignmentForm, driver: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]" placeholder="e.g. James Wilson" />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-600 mb-1">Date</label>
                    <input type="date" value={assignmentForm.date || ''} onChange={e => setAssignmentForm({...assignmentForm, date: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]" />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-gray-600 mb-1">Route Time</label>
                    <input type="time" value={assignmentForm.routeTime || ''} onChange={e => setAssignmentForm({...assignmentForm, routeTime: e.target.value})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]" />
                  </div>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-600 mb-1">Route Type</label>
                    <select value={assignmentForm.routeType || 'PICKUP'} onChange={e => setAssignmentForm({...assignmentForm, routeType: e.target.value as Assignment['routeType']})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]">
                      <option value="PICKUP">Pickup</option>
                      <option value="DROP_OFF">Drop Off</option>
                    </select>
                  </div>
                  <div>
                    ...
                    <select value={assignmentForm.status || 'SCHEDULED'} onChange={e => setAssignmentForm({...assignmentForm, status: e.target.value as Assignment['status']})} className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-2xl text-sm focus:outline-none focus:ring-2 focus:ring-[#ff3600]/20 focus:border-[#ff3600]">
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl">
                  <input type="checkbox" id="recurring" checked={assignmentForm.recurring || false} onChange={e => setAssignmentForm({...assignmentForm, recurring: e.target.checked})} className="w-4 h-4 text-[#ff3600] rounded" />
                  <label htmlFor="recurring" className="text-sm font-bold text-gray-600">Recurring Assignment</label>
                </div>
              </div>

              <div className="flex items-center gap-3 mt-6">
                <button onClick={handleSave} className="flex-1 bg-[#ff3600] text-white py-3 rounded-2xl font-bold hover:bg-[#e63200] transition-colors">{editingAssignment ? 'Save Changes' : 'Create Assignment'}</button>
